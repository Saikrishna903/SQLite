...
data_base_name_with_table_name = "DB_name_with_table_name.cfg"
...
try:
	with open(data_base_name_with_table_name) as data_base_name_with_table_name_object:
		DB_info = data_base_name_with_table_name_object.read()
		DB_info = eval(DB_info)
	data_base_name_with_table_name_object.close()
except FileNotFoundError:
	print(file_not_found_error_message)
connection = sqlite3.connect(DB_info[0])
field_names_with_data_type = connection.execute("PRAGMA table_info(" + DB_info[1] + ")")
field_names = []
...
def create_record():
	...
	insert_query = "INSERT INTO " + DB_info[1] + " VALUES" + str(field_values_tuple)
	execute_query(insert_query)
	print("Details are saved successfully.")

def print_records():
	cousor_object = connection.execute("SELECT * FROM " + DB_info[1] + " WHERE Status = 'A'")
...
def search_record():
	...
	cousor_object = connection.execute("SELECT * FROM " + DB_info[1] + " WHERE Status = 'A' AND " + str(field_names[0]) + " = " + str(key_value))
...
def update_record():
	...
	update_query = "UPDATE " + DB_info[1] + " SET " + str(field_name) + " = " + str(new_field_value) + " WHERE Status = 'A' AND " + str(field_names[0]) + " = " + str(key_value)
...
def delete_record():
	key_value = get_key_value()
	delete_query = "UPDATE " + DB_info[1] + " SET Status = 'D' WHERE " + str(field_names[0]) + " = " + str(key_value)
